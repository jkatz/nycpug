...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table(u'core_opinion', (
            ...
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(related_name='opinions', to=orm['account.User'])),
            ('proposal', self.gf('django.db.models.fields.related.ForeignKey')(related_name='opinions', to=orm['core.Proposal'])),
            ('is_recommended', self.gf('django.db.models.fields.BooleanField')(default=False)),
        ...
        # Adding unique constraint on 'Opinion', fields ['user', 'proposal']
        db.create_unique(u'core_opinion', ['user_id', 'proposal_id'])

        # Adding field 'Proposal.status'
        db.add_column(u'core_proposal', 'status',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=255, null=True, blank=True),
                      keep_default=False)
        # migrate all of the old "core_proposal.accepted" storage to new format
        # old conferences
        db.execute("""
        UPDATE core_proposal p
        SET status = (CASE p.accepted WHEN TRUE THEN 'accepted' ELSE 'declined' END)
        FROM core_conference c
        WHERE
            p.conference_id = c.id AND
            NOT c.active
        """)
        # active conferences
        db.execute("""
        UPDATE core_proposal p
        SET status = (CASE p.accepted WHEN TRUE THEN 'accepted' ELSE '' END)
        FROM core_conference c
        WHERE
            p.conference_id = c.id AND
            c.active
    ...
    def backwards(self, orm):
        # Removing unique constraint on 'Opinion', fields ['user', 'proposal']
        db.delete_unique(u'core_opinion', ['user_id', 'proposal_id'])

        # Deleting model 'Opinion'
        db.delete_table(u'core_opinion')

        # Deleting field 'Proposal.status'
        db.delete_column(u'core_proposal', 'status')
