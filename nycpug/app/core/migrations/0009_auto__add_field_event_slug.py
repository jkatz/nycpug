...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column(u'core_event', 'slug',
                      self.gf('django.db.models.fields.SlugField')(max_length=1024, null=True, blank=True),
                      keep_default=False)
        # make slugs for all the old events
        for event in orm['core.Event'].objects.all():
            event.slug = slugify(event.event_title)
            event.save()

    def backwards(self, orm):
        # Deleting field 'Event.slug'
        db.delete_column(u'core_event', 'slug')
